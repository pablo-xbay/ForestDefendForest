...
new P5(p5 => {
  ...
  window.onkeyup = function(e) { STATE.spacePressed = false }
  window.onkeydown = function(e) { STATE.spacePressed = true }
  ...
  function endGame () {
    const padding = 15
    const finalScore = STATE.forestPower + STATE.score
    p5.fill('#535353')
    p5.textAlign(p5.CENTER)
    p5.textFont(PressStartFont)
    p5.textSize(12)
    p5.text('DEFEND SCORE: ' + finalScore, (p5.width / 2), (p5.height / 2 - p5.textSize() / 2 - padding))

    STATE.isRunning = false
    p5.noLoop()
    celerx.submitScore(finalScore)
  }
  ...
  function drawForestPower () {
    ...
    //()
    STATE.forestPower -= 1;
    if (STATE.forestPower === 0) {
      STATE.gameOver = true;
      p5.text(('Forest, you lost!').padStart(3, '0'), p5.width/2, p5.textSize()+70)
    }
    p5.text(('Forest power left: ' + STATE.forestPower + '').padStart(3, '0'), p5.width/2, p5.textSize()+100)
  }
  function drawWin () {
    p5.fill('#535353')
    p5.textAlign(p5.LEFT);
    p5.textFont(PressStartFont)
    p5.textSize(24)
    p5.text(('Forest, you win!').padStart(3, '0'), p5.width/2, p5.textSize()+70)
  }
  function drawBirds () {
    const { birds } = STATE
  ...
  p5.draw = () => {
    ...
    if (STATE.dino && STATE.dino.fhits([STATE.cacti[0], STATE.birds[0]])) {
      STATE.dino.jump()
        jumpBtn.tap();
    }

    if (STATE.dino && STATE.dino.hits([STATE.cacti[0], STATE.birds[0]]) && STATE.spacePressed) {
      drawWin()
      STATE.gameOver = true
    }
